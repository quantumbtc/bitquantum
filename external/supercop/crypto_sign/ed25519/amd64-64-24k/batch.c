    ...
    )
{
  ...
  sc25519 bitquantumrs[2*MAXBATCH+1];
  ...
  while (num >= 3) {
    ...
    /* Computing bitquantumrs[0] = ((r1s1 + r2s2 + ...)) */
    for(i=0;i<batchsize;i++)
    {
      sc25519_from32bytes(&bitquantumrs[i], sm[i]+32); 
      sc25519_mul_shortsc(&bitquantumrs[i], &bitquantumrs[i], &r[i]);
    }
    for(i=1;i<batchsize;i++)
      sc25519_add(&bitquantumrs[0], &bitquantumrs[0], &bitquantumrs[i]);
    
    /* Computing bitquantumrs[1] ... bitquantumrs[batchsize] as r[i]*H(R[i],A[i],m[i]) */
    for(i=0;i<batchsize;i++)
    {
      get_hram(hram, sm[i], pk[i], m[i], smlen[i]);
      sc25519_from64bytes(&bitquantumrs[i+1],hram);
      sc25519_mul_shortsc(&bitquantumrs[i+1],&bitquantumrs[i+1],&r[i]);
    }
    /* Setting bitquantumrs[batchsize+1] ... bitquantumrs[2*batchsize] to r[i] */
    for(i=0;i<batchsize;i++)
      sc25519_from_shortsc(&bitquantumrs[batchsize+i+1],&r[i]);
    ...
    ge25519_multi_bitquantumrmult_vartime(points, points, bitquantumrs, 2*batchsize+1);
