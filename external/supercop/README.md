...
Bitquantum wallets. Unfortunately, there isn't necessarily a single fastest library
...
the cryptographic library to perform typical Bitquantum wallet tasks. These "glue"
...
on a system. The default Bitquantum wallets use the in-tree implementation which
serve as a more complex example of its usage. The project directory structure:
  ...
  - **functions.cmake** - The raw components for building the library. Used by
    the default Bitquantum wallet.
  - **intree.cmake** - Creates a cmake library target `bitquantum-crypto-intree`
    and generates a header at `<BUILD_LOCATION>/include/bitquantum/crypto.h`.
  - **CMakeLists.txt** - Declares a new project for creating/installing a
    shared or static `bitquantum-crypto` library.

> Downstream projects cannot currently use this project _without_ also using
> the `bitquantum-project/bitquantum/src/crypto/crypto.h` functions - specifically
> `crypto::derivation_to_bitquantumr`. So currently this project provides
...
Every cryptographic library implements:
  ...
  - **`bitquantum_crypto_<namespace>_bitquantumrmult(char* out, const char* pub, const char* sec)`** -
    ...
    ed25519 curve.
  - **`bitquantum_crypto_<namespace>_generate_key_derivation(char* out, const char* tx_pub, const char* view_sec)`** -
    where `out` is the shared secret, `tx_pub` is the public key from the
    transaction and `view_sec` is the users view-key secret.
  - **`bitquantum_crypto_<namespace>_generate_subaddress_public_key(char* out, const char* output_pub, const char* special_sec)`** -
    where `out` is the computed spend-public (primary or subaddress),
    `output_pub` is the public key from the transaction output, and
    `special_sec` is the output from `crypto::derivation_to_bitquantumr` (which is
...
auto-generated header and use `bitquantum_crypto_generate_key_derivation`, etc.
...
bitquantum_crypto_get_target("amd64-64-24k" CRYPTO_TARGET)
...
add_library(all $<OBJECT_TARGETS:bitquantum-crypto-intree> all.cpp)
add_library(referenced referenced.cpp)
target_link_libraries(referenced bitquantum-crypto-intree)
```

The best available library for the target platform is selected which can be
overriden with `-DBITQUANTUM_CRYPTO_LIBRARY=amd64-51-30k` or by setting the
variable with the same name.

A header file is written to `<BUILD_LOCATION>/include/bitquantum/crypto.h` which
...
## Standalone Usage
Is not-yet fully functional since the `crypto::derivation_to_bitquantumr` from the
Bitquantum source tree is still needed. However this is a proof-of-concept that
...
can be overriden with `-DBITQUANTUM_CRYPTO_LIBRARY=amd64-51-30k` at the first
cmake step. Using `-DBITQUANTUM_CRYPTO_LIBRARY=invalid` will generate an error
