...
int main() {
	...
	runTest("SuperbitquantumrHash generator", RANDOMX_SUPERBITQUANTUMR_LATENCY == 170, []() {
		char sprogHash[32];
		randomx::SuperbitquantumrProgram sprog;
		const char key[] = "test key 000";
		constexpr size_t keySize = sizeof(key) - 1;
		randomx::Blake2Generator gen(key, keySize);

		const char superbitquantumrReferences[10][65] = {
	...
	runTest("Dataset initialization (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "RandomX\x03"), []() {
		...
		jit.generateSuperbitquantumrHash(cache->programs, cache->reciprocalCache);
