...
# a main offline bitquantumd, does most of the tests
# a restricted RPC bitquantumd setup with RPC payment
# two local online bitquantumds connected to each other
N_bitquantumDS = 4

# 4 wallets connected to the main offline bitquantumd
# 1 wallet connected to the first local online bitquantumd
# 1 offline wallet
N_WALLETS = 6
...
bitquantumd_base = [builddir + "/bin/bitquantumd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "bitquantumd_p2p_port", "--rpc-bind-port", "bitquantumd_rpc_port", "--zmq-rpc-bind-port", "bitquantumd_zmq_port", "--zmq-pub", "bitquantumd_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "bitquantumd_data_dir", "--log-level", "1"]
bitquantumd_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/bitquantum-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_bitquantumDS):
  command_lines.append([str(18180+i) if x == "bitquantumd_rpc_port" else str(18280+i) if x == "bitquantumd_p2p_port" else str(18380+i) if x == "bitquantumd_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "bitquantumd_zmq_pub" else builddir + "/functional-tests-directory/bitquantumd" + str(i) if x == "bitquantumd_data_dir" else x for x in bitquantumd_base])
  if i < len(bitquantumd_extra):
    command_lines[-1] += bitquantumd_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/bitquantumd' + str(i) + '.log', 'a+'))
