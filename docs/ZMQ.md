# The Current/Future Status of ZMQ in bitquantum

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. bitquantum
...
matching supported by both bitquantum and ZMQ. The `format`, `context` and `event`
...
The bitquantum daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The bitquantum daemon also ensures that
...
The bitquantum daemon will send a `txpool_add` pub exactly once for each
