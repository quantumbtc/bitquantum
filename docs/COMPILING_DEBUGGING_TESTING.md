...
The intended audience are developers, who want to leverage newly added tricks to bitquantum via `CMake`. The document will lower the entry point for these developers.
...
bitquantum can be compiled via the main `Makefile`, using one of several targets listed there.
...
It is possible to extract these `CMake` calls and modify them for your specific needs. For example, a minimal external cmake command to compile bitquantum, executed from within a newly created build directory could look like:

`cmake -S "$DIR_SRC" -DCMAKE_BUILD_TYPE=Release && make`

where the variable `DIR_SRC` is expected to store the path to the bitquantum source code.
...
The additional artifact of the above call is the `bitquantum.cbp` Code::Blocks project file in the build directory.
...
After a successful build, open the `bitquantum.cbp` with CB. From the CB's menu bar select the target, that you want debug. Assuming these are unit tests:
...
https://github.com/bitquantum-project/bitquantum/pull/7160

### Faster core_tests with caching
https://github.com/bitquantum-project/bitquantum/pull/5821

### Precompiled headers
https://github.com/bitquantum-project/bitquantum/pull/7216

### Unity builds
https://github.com/bitquantum-project/bitquantum/pull/7217
