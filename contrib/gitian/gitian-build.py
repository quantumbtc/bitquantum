...
gsigs = 'https://github.com/bitquantum-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('bitquantum'):
        # Remove the potentially stale bitquantum dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'bitquantum', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'bitquantum'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/bitquantum/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'bitquantum='+args.commit, '--url', 'bitquantum='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/bitquantum-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/bitquantum/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/bitquantum/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/bitquantum-project/bitquantum', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/bitquantum', exist_ok=True)
    os.chdir('builder/inputs/bitquantum')
    if args.pull:
